...
TOOL_DECLARATIONS = [
    {
        ...
        "description": (
            "Generates a complete website (HTML, CSS, JS) from a brief, writes it "
            "into the developer workspace, starts a local preview server, and opens "
            "the project in VS Code and Chrome."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "site_name": {
                    "type": "string",
                    "description": "The name/title of the website or project.",
                },
                "brief": {
                    "type": "string",
                    "description": "A description of what the website should be/do.",
                },
                "workspace_path": {
                    "type": "string",
                    "description": "Optional folder path to write the project into. Defaults to the configured developer workspace.",
                },
            },
...
def _generate_spec(parameters: dict[str, Any]) -> dict[str, Any]:
    ...
    site_name = _sanitize_text(
        str(parameters.get("site_name") or parameters.get("title") or "Jarvis Project"),
        "Jarvis Project",
    )
    if not brief:
...
def jarvis_tool_generate_website(**kwargs: Any) -> str:
    """Wrapper so main.py can call this tool directly with keyword arguments."""
    return website_builder(parameters=kwargs)


def jarvis_tool_generate_website(**kwargs):
    """Wrapper so main.py can call this tool directly with keyword arguments."""
    return website_builder(parameters=kwargs)
