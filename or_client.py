...
def _load_api_key() -> str:
    try:
        ...
        raise RuntimeError(f"api_keys.json not found at: {API_KEY_PATH}") from e
    except json.JSONDecodeError as e:
        raise RuntimeError(f"api_keys.json at {API_KEY_PATH} is not valid JSON: {e}") from e
    except Exception as e:
        raise RuntimeError(f"Failed to load OpenRouter API key: {e}") from e
